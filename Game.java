...
public class Game {
	...
	private Player[] playerCopy(Player[] initial) {
		Player[] temp = new Player[initial.length];
		for(int i=0;i<temp.length;i++) {
			temp[i] = Player.getInstance(initial[i]);
		}
		return temp;
	}
	...
	private Game(Game game) {
		this.player=playerCopy(game.player);
